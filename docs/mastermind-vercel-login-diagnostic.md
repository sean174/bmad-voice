...
This report covers the Vercel Mastermind app at `mastermind.seanthomas.co` and `bmad-voice.vercel.app`. Older references to `mastermind.seanthomas.com` are stale unless that domain is deliberately reattached to the same Vercel deployment.
...
- The start screen now shows a non-secret health/session diagnostic. It reports reachability/configuration booleans and, for a valid signed-in session only, a bridge status enum. It never prints tokens, bridge URLs, database strings, or bridge data.
- The login and start screens show a visible build stamp. Current expected build: `2026-06-07-2`.
- `https://mastermind.seanthomas.co/?reset=1` is a public browser/PWA recovery URL. It clears `localStorage` and `sessionStorage`, then redirects to `/` with cache-busting query parameters so iOS Safari or an installed PWA has to request the current app shell.
...
3. **Login succeeds but chat job/runtime config is missing or broken.** `/api/health` can return 200 while one of its `checks` booleans is false. A signed-in `/api/health?bridge=1` check can also show `commandCenterContextLive.status` as `unauthorized`, `bad_status`, `invalid_json`, `empty`, `timeout`, or `fetch_failed`. If `POSTGRES_URL` is absent, `/api/chat-job` returns `400 No database configured` after auth. If Hermes/Anthropic or bridge runtime config is bad, chat jobs can fail later with `Failed to process chat job`, `AI service error`, or a queued job that never completes.
...
- `GET /api/health` returns 200 but the app still fails: unauthenticated health only confirms public app reachability and env presence booleans, not that the saved browser token is current or that the chat worker completed a job.
- `GET /api/health?bridge=1` with `x-session-token` returns `commandCenterContextLive.status` other than `loaded`: auth passed but the server-side Command Center bridge is not delivering usable live context to the Mastermind runtime. `unauthorized` points to a token mismatch, `timeout` or `fetch_failed` points to bridge reachability, `bad_status` points to a non-2xx bridge response, `invalid_json` points to a bridge response contract issue, and `empty` means the bridge returned metadata-only or empty context.
...
1. Open this exact URL on the phone: `https://mastermind.seanthomas.co/?reset=1`
2. Wait for it to redirect to `https://mastermind.seanthomas.co/?v=2026-06-07-2&t=...`.
...
`GET /api/health` is public and returns booleans plus the request host. It does not print bridge URLs, tokens, API keys, database strings, or secret values.

Use it to separate basic Vercel app reachability, attached domain, and env presence from session-token failures:

```bash
curl -fsS https://bmad-voice.vercel.app/api/health
curl -fsS https://mastermind.seanthomas.co/api/health
```
...
After logging in, run the bridge probe without printing the token:

```bash
curl -fsS 'https://mastermind.seanthomas.co/api/health?bridge=1' \
  -H "x-session-token: $TOKEN"
```

Expected result: `sessionAuthenticated` is true and `checks.commandCenterContextLive.status` is `loaded`. Any other status is a live bridge delivery problem, even if `commandCenterContextConfigured` is true.
